...
class WxApi {
  ...
  static Future<List<Map<String, dynamic>>> nationalSmart({
    ...
    required int Wind ≥ mphString? state,
  }) async {
    ...
    final common = <String, String>{
      ...
      'max_zones': '$maxZones',
      'threshold': '$threshold',
      'wind_mph': '$Wind ≥ mphhorizon_hours': '',
    ...
    for (final hv in horizonVariants) {
      for (final fmt in formats) {
        ...
        try {
          ...
          debugPrint(
              '[WxApi] ${rows.length} rows via ${hv.keys.first}=$horizonHours format=$fmt');
  ...
  static Future<List<Map<String, dynamic>>> _decodeToRows(
      String fmt, String body) async {
