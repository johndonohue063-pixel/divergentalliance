...
class _WeatherCenterState extends State<WeatherCenter> {
  ...
  int get _Wind ≥ mphint.tryParse(_windCtl.text) ?? 35;
  ...
  num _mphOf(Map<String, dynamic> m) {
    for (final v in [
      m['Peak Wind ≥ mphm['Wind ≥ mphm['Gust mph'],
      m['peak_wind_mph'],
      m['wind_mph'],
      m['gust_mph'],
      m['wind_gust_max'],
  ...
  List<Map<String, dynamic>> _applyClientFilters(
    ...
    list = list.where((m) {
      final w = _mphOf(m);
      return w < 0 ? true : w >= _Wind ≥ mphtoList();
  ...
  Future<void> _runReport() async {
    ...
    Future<List<Map<String, dynamic>>> fetch(String region, int hours) {
      return WxApi.nationalSmart(
        ...
        threshold: 12,
        horizonHours: hours,
        Wind ≥ mph_Wind ≥ mphfinal attempts = <int>{48, 24, _horizonHours, 72, 96, 120}.toList();
    ...
    try {
          ...
          if (rows.isNotEmpty) {
            setState(() {
              ...
              _note = 'Showing top ${rows.length} , region: $_regionParam'
                  ...
                  'wind = $_Wind ≥ mphmph , ${hh ~/ 24} day(s) out';
            });
      ...
      setState(() {
        _note =
            'No rows found. Try another region, reduce Days out, or lower Wind ≥ mphcatch (e) {
      if (!mounted || myToken != _runToken) return;
  ...
  Future<void> _exportCsv() async {
    var uri = WxApi.nationalCsvUri(
      ...
      Wind ≥ mph_Wind ≥ mphstate: _stateParam.isEmpty ? null : _stateParam,
    );
...
class _ResultTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final wind =
        '${row['Peak Wind ≥ mphrow['Wind ≥ mphrow['Gust mph'] ?? ''}';
