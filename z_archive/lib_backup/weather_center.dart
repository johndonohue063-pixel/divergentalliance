...
class _WeatherCenterState extends State<WeatherCenter> {
  ...
  void initState() {
    super.initState();
    WxClient.discover(); // sets base URL properly per platform
  }
  ...
  String _friendlyError(Object e) {
    ...
    if (s.contains('TimeoutException')) {
      return 'Server took too long to respond. Make sure the backend is running and the app is pointing to 10.0.2.2:8010 (Android emulator).';
    }
    if (s.contains('SocketException') || s.contains('Connection refused')) {
      return 'Can’t reach the backend. On the Android emulator use 10.0.2.2:8010. On a physical device, run: adb reverse tcp:8010 tcp:8010.';
    }
    return s;
  }
  ...
  List<Map<String, dynamic>> _applyClientFilters(List<Map<String, dynamic>> list) {
    ...
    // sort by predicted customers out (desc) and clip to 50
  ...
  Future<void> _runReport() async {
    ...
    try {
      ...
      final data = await WxClient.nationalCsv(
        region:   _regionParam,   // 'national' | 'southeast' | etc
        maxZones: 50,
        windMphMin: _windMph,     // UI "Wind mph (min)"
        daysOut:  _days,          // UI "Days out"
      ...
      // Region subset if needed, then client filters
                            ...
                            child: DropdownButtonHideUnderline(
                              child: DropdownButton<String>(
                                ...
                                onChanged: _stateDisabled
                                  ...
                                  _statePretty = v!;
                                  if (_stateParam.isNotEmpty) {
                                    _regionPretty = 'National'; // keep mutual exclusion clean
                                  }
                                }),
